...
class DefaultPriceFormatter implements PriceFormatterInterface
{
    ...
    {
        $convertedValue = $this->value / (int) $this->currency->factor;
    ...
    {
        $convertedValue = $this->value / (int) $this->currency->factor / $this->unitQty;
    ...
    public function formatValue(int|float $value, ?string $locale = null, int $formatterStyle = NumberFormatter::CURRENCY, ?int $decimalPlaces = null, bool $trimTrailingZeros = true): mixed
    {
        if (! $locale) {
